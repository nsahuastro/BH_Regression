...
:rtype: float sd with the mean deviation.
	...
	sd=1./(N)* numpy.sum((y-a*x-b)**2)
	sd=numpy.sqrt(sd)
	
	return sd
	
def fitexyiscat(x,y,errx,erry,a,b):	
	"""
FITEXY scatter, see Graham & Driver (2007) Equation 1
October 19, 2018
	"""
	...
	chisq = numpy.sum((y-a*x-b)**2/(erry**2+a**2*errx**2+epsilon**2))
...
def myintscat(x,y,errx,erry,a,b):
	"""
Traditional method of computing intrinsic scatter.
	
Usage:

>>> sd=myintscat(x,y,errx,erry,a,b)
...
v1 Jun 27th 2017
	...
	return numpy.median(intscati),astropy.stats.median_absolute_deviation(intscati)
	#return numpy.mean(intscati),numpy.std(intscati)
